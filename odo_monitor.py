...
def _normalize_date_str(raw_date: str) -> str:
    """Chuyển đổi chuỗi ngày bất kỳ về định dạng DD/MM/YYYY."""
    if not raw_date:
        return ""
    s = str(raw_date).strip().split(" ")[0] # Bỏ giờ nếu có
...
def _match_master_kho(kho_input: str, master_khos: list) -> str:
    ...
    keywords = {
        ...
        "buôn ma thuột": "Kho Giao Hàng Nặng - Buôn Ma Thuột - Đắk Lắk",
        "cam ranh": "Kho Giao Hàng Nặng - Cam Ranh - Khánh Hòa",
        "phân thiết": "Kho Giao Hàng Nặng - Phan Thiết - Bình Thuận",
    ...
    for kw, target_kho in keywords.items():
        if kw in k_clean:
            # Match against master_khos if available
            for mk in master_khos:
                if target_kho.lower() in mk.lower():
                    return mk
            return target_kho

    # Fallback fuzzy substring match against master_khos
    for mk in master_khos:
...
def fetch_google_sheet_odo_data(force_refresh: bool = False) -> dict:
    ...
    Đếm số dòng ODO theo (Ngày + Kho). Dùng Cache 3 phút.
    ...
    try:
        ...
        # 1. Tìm tên tab tháng hiện tại (ví dụ 'THÁNG 7', 'Tháng 7', 'THÁNG 8')
        ...
        if not target_tab:
            # Fallback to any tab containing THÁNG or THANG or use the last active month tab
        ...
        for r in rows[1:]:
            ...
            date_norm = _normalize_date_str(raw_date)
        ...
        print(f"[ODO MONITOR] Cached {len(res_dict)} (date, kho) counts from Google Sheet.")
        return res_dict
...
def get_xe_daily_breakdown(date_str: str) -> tuple:
    ...
    for r in records:
        r_date = _normalize_date_str(r.get("ngay", ""))
        if r_date == date_norm:
...
def calculate_odo_status(target_date: str = None, force_refresh: bool = False) -> dict:
    """
    Tính toán trạng thái ODO của tất cả các kho theo target_date (mặc định là hôm nay).
    Công thức: expectedOdo = tongXe + xeTangCuong - xeOff
    Returns dict chứa:
      - summary: { total_khos, du_khos, thieu_khos, total_xe_thieu, total_xe_thua }
      - details: [ { kho, tong_xe, xe_off, xe_tc, expected_odo, actual_odo, status, diff }, ... ]
    """
    if not target_date:
        target_date = datetime.now().strftime("%d/%m/%Y")
    else:
        target_date = _normalize_date_str(target_date)

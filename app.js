...
// ================================================================
// ===== MODULE: BÁO CÁO ODO GXT ==================================
// ================================================================

let odoData = [];          // Raw data from API
let odoKpi = {};           // KPI summary from API
let odoCurrentTab = 'overview';

// ---- Setup nav click for ODO section ----
function setupOdoNavigation() {
    const navOdo = document.getElementById('nav-odo');
    if (!navOdo) return;
    navOdo.addEventListener('click', (e) => {
        e.preventDefault();
        // Deactivate all other sections
        document.querySelectorAll('.section').forEach(s => s.classList.remove('active'));
        document.querySelectorAll('.nav-item').forEach(n => n.classList.remove('active'));
        // Activate ODO section
        document.getElementById('section-odo').classList.add('active');
        navOdo.classList.add('active');
        navOdo.style.background = 'white';
        navOdo.style.borderLeft = '3px solid var(--orange)';
        // Update page title
        const pt = document.getElementById('page-title');
        const ps = document.getElementById('page-subtitle');
        if (pt) pt.textContent = 'Báo Cáo ODO GXT';
        if (ps) ps.textContent = 'Theo dõi báo cáo ODO xe GXT Miền Trung từ 02/06/2026';
        // Load data if not loaded
        if (odoData.length === 0) loadOdoReport(false);
    });
}

// ---- Load ODO data from API ----
async function loadOdoReport(force = false) {
    const btn = document.getElementById('odo-refresh-btn');
    if (btn) { btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Đang tải...'; btn.disabled = true; }

    try {
        const url = `${API}/odo/report${force ? '?force=true' : ''}`;
        const resp = await fetch(url);
        const json = await resp.json();

        odoData = json.data || [];
        odoKpi  = json.kpi  || {};
        const lastSync = json.last_sync;

        // Update sync time display
        const syncEl = document.getElementById('odo-last-sync');
        if (syncEl && lastSync) {
            const d = new Date(lastSync * 1000);
            syncEl.innerHTML = `<i class="fa-solid fa-clock"></i> Cập nhật: ${d.toLocaleTimeString('vi-VN')}`;
        }

        // Populate kho filter dropdown in history tab
        populateOdoKhoFilter();

        // Render current visible tab
        renderCurrentOdoTab();

        // Update nav badge
        const badge = document.getElementById('nav-odo-count');
        if (badge) {
            const nc = odoKpi.need_check || 0;
            badge.textContent = nc;
            badge.style.display = nc > 0 ? 'inline-block' : 'none';
        }
        // Update alert badge on tab button
        updateOdoAlertBadge();

    } catch (err) {
        console.error('[ODO] Load failed:', err);
    } finally {
        if (btn) { btn.innerHTML = '<i class="fa-solid fa-rotate-right"></i> Làm mới'; btn.disabled = false; }
    }
}

// ---- Switch between tabs ----
function switchOdoTab(tab) {
    odoCurrentTab = tab;
    // Update tab button styles
    ['overview', 'alerts', 'history'].forEach(t => {
        const btn = document.getElementById(`odo-tab-${t}`);
        const panel = document.getElementById(`odo-panel-${t}`);
        if (!btn || !panel) return;
        if (t === tab) {
...
function renderCurrentOdoTab() {
    if (odoCurrentTab === 'overview') {
        renderOdoKpiCards();
        renderOdoKhoSummaryTable();
    } else if (odoCurrentTab === 'alerts') {
        renderOdoAlertKpi();
        renderOdoAlerts();
    } else if (odoCurrentTab === 'history') {
        renderOdoHistory();
    }
}

// ---- Render KPI cards (Tab 1) ----
function renderOdoKpiCards() {
    const container = document.getElementById('odo-kpi-cards');
    if (!container) return;

    const kpi = odoKpi;
    const pct = kpi.pct_complete || 0;
    const pctColor = pct >= 80 ? 'var(--green)' : pct >= 50 ? 'var(--orange)' : 'var(--red)';

    container.innerHTML = `
        <div class="stat-card green-card" style="padding:18px;">
            <div class="stat-icon green" style="width:42px;height:42px;font-size:18px;"><i class="fa-solid fa-file-circle-check"></i></div>
            <div class="stat-details">
                <h3>Tổng báo cáo ODO</h3>
                <h2 style="color:var(--green)">${(kpi.total_reports || 0).toLocaleString()}</h2>
                <p class="stat-sub">Từ 02/06/2026 trở đi</p>
            </div>
        </div>
        <div class="stat-card" style="padding:18px;border-left:4px solid var(--blue);">
            <div class="stat-icon blue" style="width:42px;height:42px;font-size:18px;"><i class="fa-solid fa-warehouse"></i></div>
            <div class="stat-details">
                <h3>Số kho có báo cáo</h3>
                <h2 style="color:var(--blue)">${kpi.total_khos || 0}</h2>
                <p class="stat-sub">Kho phát sinh báo cáo</p>
            </div>
        </div>
        <div class="stat-card" style="padding:18px;border-left:4px solid var(--purple);">
            <div class="stat-icon purple" style="width:42px;height:42px;font-size:18px;background:#EDE9FE;color:#7C3AED;"><i class="fa-solid fa-truck"></i></div>
            <div class="stat-details">
                <h3>Số xe có báo cáo</h3>
                <h2 style="color:#7C3AED">${kpi.total_plates || 0}</h2>
                <p class="stat-sub">Biển số xe duy nhất</p>
            </div>
        </div>
        <div class="stat-card green-card" style="padding:18px;">
            <div class="stat-icon green" style="width:42px;height:42px;font-size:18px;"><i class="fa-solid fa-circle-check"></i></div>
            <div class="stat-details">
                <h3>Báo cáo hoàn thành</h3>
                <h2 style="color:var(--green)">${kpi.complete || 0}</h2>
                <p class="stat-sub">Đầy đủ KM + ảnh</p>
            </div>
        </div>
        <div class="stat-card red-card alert-card" style="padding:18px;">
            <div class="stat-icon red" style="width:42px;height:42px;font-size:18px;"><i class="fa-solid fa-circle-exclamation"></i></div>
            <div class="stat-details">
                <h3>Cần kiểm tra</h3>
                <h2 style="color:var(--red)">${kpi.need_check || 0}</h2>
                <p class="stat-sub">Thiếu KM, ảnh, bất thường</p>
            </div>
        </div>
        <div class="stat-card gold-card" style="padding:18px;">
            <div class="stat-icon" style="width:42px;height:42px;font-size:18px;background:#FFFDE7;color:#F59E0B;"><i class="fa-solid fa-percent"></i></div>
            <div class="stat-details">
                <h3>Tỷ lệ hoàn thành</h3>
                <h2 style="color:${pctColor}">${pct}%</h2>
                <p class="stat-sub" style="color:${pctColor}">${pct >= 80 ? '✅ Tốt' : pct >= 50 ? '⚠️ Cần cải thiện' : '🔴 Thấp'}</p>
            </div>
        </div>
    `;
}

// ---- Render Kho Summary Table (Tab 1) ----
function renderOdoKhoSummaryTable() {
    const tbody = document.getElementById('odo-kho-summary-body');
    const countEl = document.getElementById('odo-kho-summary-count');
    if (!tbody) return;

    // Filter data
    const dateFilter = document.getElementById('odo-filter-date')?.value || '';
    const khoFilter  = (document.getElementById('odo-filter-kho')?.value || '').toLowerCase();
    const statFilter = document.getElementById('odo-filter-status')?.value || '';

    let filtered = odoData;
    if (dateFilter) {
        const [y, m, d] = dateFilter.split('-');
        const targetDate = `${d}/${m}/${y}`;
        filtered = filtered.filter(r => r.ngay === targetDate);
    }
    if (khoFilter) filtered = filtered.filter(r => (r.ten_kho || '').toLowerCase().includes(khoFilter));
    if (statFilter) filtered = filtered.filter(r => r._status === statFilter);

    // Group by kho
    const khoMap = {};
    filtered.forEach(r => {
        const key = r.id_kho || r.ten_kho || 'Không rõ';
        const name = r.ten_kho || r.id_kho || 'Không rõ';
        if (!khoMap[key]) khoMap[key] = { name, total: 0, complete: 0, missing_km: 0, abnormal: 0 };
        khoMap[key].total++;
        if (r._status === 'complete') khoMap[key].complete++;
        if (r._status === 'missing_km' || r._status === 'incomplete') khoMap[key].missing_km++;
        if (r._status === 'abnormal') khoMap[key].abnormal++;
    });

    const khos = Object.values(khoMap).sort((a, b) => b.total - a.total);
    if (countEl) countEl.textContent = `${khos.length} kho`;

    if (!khos.length) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--text3);padding:30px;">Không có dữ liệu phù hợp</td></tr>';
        return;
    }

    tbody.innerHTML = khos.map(k => {
        const pct = k.total > 0 ? Math.round(k.complete / k.total * 100) : 0;
        const pctColor = pct >= 80 ? 'var(--green)' : pct >= 50 ? 'var(--orange)' : 'var(--red)';
        const pctBg = pct >= 80 ? '#E8F5E9' : pct >= 50 ? '#FFF3E0' : '#FFF0F0';
        const shortName = k.name.replace('Kho Giao Hàng Nặng - ', '');
        return `<tr>
            <td style="font-weight:600;font-size:13px;">${shortName}</td>
            <td style="text-align:center;font-weight:700;color:var(--blue)">${k.total}</td>
            <td style="text-align:center;color:var(--green);font-weight:700">${k.complete}</td>
            <td style="text-align:center;color:${k.missing_km > 0 ? 'var(--red)' : 'var(--text3)'};font-weight:${k.missing_km > 0 ? '700' : '400'}">${k.missing_km || '—'}</td>
            <td style="text-align:center;color:${k.abnormal > 0 ? 'var(--red)' : 'var(--text3)'};font-weight:${k.abnormal > 0 ? '700' : '400'}">${k.abnormal || '—'}</td>
            <td style="text-align:center;">
                <span style="font-size:13px;font-weight:800;color:${pctColor};background:${pctBg};padding:3px 10px;border-radius:12px;">${pct}%</span>
            </td>
        </tr>`;
    }).join('');
}
...
// ---- Filters for Tab 1 ----
function applyOdoFilters() { renderOdoKhoSummaryTable(); }
function resetOdoFilters() {
    const ids = ['odo-filter-date', 'odo-filter-kho', 'odo-filter-status'];
    ids.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    renderOdoKhoSummaryTable();
}

// ---- Render Alert KPI (Tab 2) ----
function renderOdoAlertKpi() {
    const counts = { missing_km: 0, incomplete: 0, missing_image: 0, abnormal: 0 };
    odoData.forEach(r => {
        if (r._status === 'missing_km') counts.missing_km++;
        else if (r._status === 'incomplete') counts.incomplete++;
        else if (r._status === 'missing_image') counts.missing_image++;
        else if (r._status === 'abnormal') counts.abnormal++;
    });
    const el = id => document.getElementById(id);
    if (el('odo-cnt-missing-km')) el('odo-cnt-missing-km').textContent = counts.missing_km;
    if (el('odo-cnt-incomplete')) el('odo-cnt-incomplete').textContent = counts.incomplete;
    if (el('odo-cnt-missing-img')) el('odo-cnt-missing-img').textContent = counts.missing_image;
    if (el('odo-cnt-abnormal')) el('odo-cnt-abnormal').textContent = counts.abnormal;
}

function updateOdoAlertBadge() {
    const total = odoData.filter(r => r._status !== 'complete').length;
    const badge = document.getElementById('odo-alert-badge');
    if (badge) badge.textContent = total;
}

// ---- Render Alerts Table (Tab 2) ----
function renderOdoAlerts() {
    const tbody = document.getElementById('odo-alert-body');
    const totalBadge = document.getElementById('odo-alert-total-badge');
    if (!tbody) return;

    const search    = (document.getElementById('odo-alert-search')?.value || '').toLowerCase();
    const typeFilter = document.getElementById('odo-alert-filter-type')?.value || '';

    const alertStatuses = ['missing_km', 'incomplete', 'missing_image', 'abnormal', 'invalid_date', 'missing_date'];
    let alerts = odoData.filter(r => alertStatuses.includes(r._status));

    if (typeFilter) alerts = alerts.filter(r => r._status === typeFilter);
    if (search) alerts = alerts.filter(r =>
        (r.ten_kho || '').toLowerCase().includes(search) ||
        (r.bien_so || '').toLowerCase().includes(search) ||
        (r.ncc || '').toLowerCase().includes(search)
    );

    if (totalBadge) totalBadge.textContent = `${alerts.length} cảnh báo`;

    const statusConfig = {
        missing_km:    { label: '⚠️ Thiếu số KM', color: 'var(--red)', bg: '#FEF2F2', level: '🔴 Khẩn', suggest: 'Nhắc tài xế gửi lại ảnh đồng hồ rõ nét hoặc nhập trực tiếp vào Sheet.' },
        incomplete:    { label: '🟡 Thiếu KM đi/về', color: 'var(--orange)', bg: '#FFF3E0', level: '🟠 Cần KT', suggest: 'Kiểm tra lại báo cáo, yêu cầu bổ sung số KM còn thiếu.' },
        missing_image: { label: '📷 Thiếu ảnh ODO', color: '#7C3AED', bg: '#EDE9FE', level: '🟡 Theo dõi', suggest: 'Yêu cầu tài xế chụp lại ảnh đồng hồ công tơ mét và gửi lại.' },
        abnormal:      { label: '📉 KM bất thường', color: 'var(--red)', bg: '#FFF0F0', level: '🔴 Kiểm tra ngay', suggest: 'Xác minh lại số ODO — KM về nhỏ hơn KM đi, nghi ngờ nhập sai.' },
        invalid_date:  { label: '❓ Ngày sai định dạng', color: 'var(--text3)', bg: '#F8F9FA', level: '🟡 Cần KT', suggest: 'Kiểm tra lại định dạng ngày trong Sheet.' },
        missing_date:  { label: '❌ Thiếu ngày', color: 'var(--text3)', bg: '#F8F9FA', level: '🔴 Thiếu', suggest: 'Bản ghi này thiếu thông tin ngày, cần kiểm tra trong Sheet.' },
    };

    if (!alerts.length) {
        tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:var(--green);padding:30px;font-weight:600;">✅ Không có cảnh báo nào — Tất cả báo cáo đều ổn!</td></tr>';
        return;
    }

    tbody.innerHTML = alerts.map(r => {
        const cfg = statusConfig[r._status] || statusConfig['missing_km'];
        const shortKho = (r.ten_kho || 'Không rõ').replace('Kho Giao Hàng Nặng - ', '');
        return `<tr style="background:${cfg.bg};">
            <td style="white-space:nowrap;font-weight:600;">${r.ngay || '??'}</td>
            <td style="font-size:12.5px;">${shortKho}</td>
            <td style="font-weight:700;font-family:monospace;">${r.bien_so || '--'}</td>
            <td style="font-size:12.5px;">${r.ncc || '--'}</td>
            <td style="color:${cfg.color};font-weight:700;">${cfg.label}</td>
            <td><span style="font-size:12px;font-weight:700;color:${cfg.color};white-space:nowrap;">${cfg.level}</span></td>
            <td style="font-size:12px;color:var(--text2);max-width:240px;">${cfg.suggest}</td>
        </tr>`;
    }).join('');
}

// ---- Populate Kho dropdown for History ----
function populateOdoKhoFilter() {
    const sel = document.getElementById('odo-hist-kho');
    if (!sel) return;
    const khos = [...new Set(odoData.map(r => r.ten_kho || r.id_kho).filter(Boolean))].sort();
    sel.innerHTML = '<option value="">Tất cả kho</option>' + khos.map(k => {
        const short = k.replace('Kho Giao Hàng Nặng - ', '');
        return `<option value="${k}">${short}</option>`;
    }).join('');
}

// ---- Render History Table (Tab 3) ----
function renderOdoHistory() {
    const tbody = document.getElementById('odo-history-body');
    const countEl = document.getElementById('odo-history-count');
    const resultEl = document.getElementById('odo-hist-result-count');
    if (!tbody) return;

    const search    = (document.getElementById('odo-hist-search')?.value || '').toLowerCase();
    const dateVal   = document.getElementById('odo-hist-date')?.value || '';
    const statusVal = document.getElementById('odo-hist-status')?.value || '';
    const khoVal    = document.getElementById('odo-hist-kho')?.value || '';

    let filtered = [...odoData]; // already sorted newest first

    if (dateVal) {
        const [y, m, d] = dateVal.split('-');
        const t = `${d}/${m}/${y}`;
        filtered = filtered.filter(r => r.ngay === t);
    }
    if (statusVal) filtered = filtered.filter(r => r._status === statusVal);
    if (khoVal) filtered = filtered.filter(r => r.ten_kho === khoVal || r.id_kho === khoVal);
    if (search) filtered = filtered.filter(r =>
        (r.ten_kho || '').toLowerCase().includes(search) ||
        (r.bien_so || '').toLowerCase().includes(search) ||
        (r.ncc || '').toLowerCase().includes(search) ||
        (r.id_kho || '').toLowerCase().includes(search)
    );

    if (countEl) countEl.textContent = `${filtered.length} bản ghi`;
    if (resultEl) resultEl.textContent = filtered.length < odoData.length ? `(Đang lọc ${filtered.length}/${odoData.length})` : '';

    if (!filtered.length) {
        tbody.innerHTML = '<tr><td colspan="12" style="text-align:center;color:var(--text3);padding:40px;">Không tìm thấy dữ liệu phù hợp</td></tr>';
        return;
    }

    const statusBadge = {
        complete:      `<span style="background:#E8F5E9;color:var(--green);font-size:11px;font-weight:700;padding:3px 8px;border-radius:6px;">✅ Hoàn thành</span>`,
        missing_km:    `<span style="background:#FEF2F2;color:var(--red);font-size:11px;font-weight:700;padding:3px 8px;border-radius:6px;">⚠️ Thiếu KM</span>`,
        incomplete:    `<span style="background:#FFF3E0;color:var(--orange);font-size:11px;font-weight:700;padding:3px 8px;border-radius:6px;">🟡 Chưa đủ</span>`,
        missing_image: `<span style="background:#EDE9FE;color:#7C3AED;font-size:11px;font-weight:700;padding:3px 8px;border-radius:6px;">📷 Thiếu ảnh</span>`,
        abnormal:      `<span style="background:#FEF2F2;color:var(--red);font-size:11px;font-weight:700;padding:3px 8px;border-radius:6px;">🔴 Bất thường</span>`,
        invalid_date:  `<span style="background:#F1F5F9;color:var(--text3);font-size:11px;font-weight:700;padding:3px 8px;border-radius:6px;">❓ Ngày sai</span>`,
        missing_date:  `<span style="background:#F1F5F9;color:var(--text3);font-size:11px;font-weight:700;padding:3px 8px;border-radius:6px;">❌ Thiếu ngày</span>`,
    };

    tbody.innerHTML = filtered.map((r, i) => {
        const shortKho = (r.ten_kho || r.id_kho || '--').replace('Kho Giao Hàng Nặng - ', '');
        const kmDi = r.km_di || '—';
        const kmVe = r.km_ve || '—';
        let kmMove = '—';
        if (r.km_di && r.km_ve) {
            try {
                const diff = parseFloat(r.km_ve.replace(/[,.]/g, '')) - parseFloat(r.km_di.replace(/[,.]/g, ''));
                if (!isNaN(diff)) {
                    const color = diff < 0 ? 'var(--red)' : diff > 500 ? 'var(--orange)' : 'var(--green)';
                    kmMove = `<span style="color:${color};font-weight:700;">${diff.toLocaleString()} km</span>`;
                }
            } catch(e) {}
        }

        const imgBtn = r.link_anh
            ? `<button onclick="openOdoImage('${r.link_anh.replace(/'/g,"\\'")}','${r.bien_so}')"
                style="background:var(--blue-bg);color:var(--blue);border:1px solid var(--blue-border);padding:4px 10px;border-radius:6px;cursor:pointer;font-size:11.5px;font-weight:600;white-space:nowrap;">
                <i class="fa-solid fa-image"></i> Xem ảnh
               </button>`
            : `<span style="color:var(--text-light);font-size:11px;">Không có</span>`;

        const rowBg = r._status === 'abnormal' ? '#FFF5F5' : r._status === 'complete' ? '' : '#FFFDF0';
        return `<tr style="background:${rowBg}">
            <td style="color:var(--text3);font-size:12px;">${i + 1}</td>
            <td style="white-space:nowrap;font-weight:600;">${r.ngay || '--'}</td>
            <td style="font-size:12.5px;max-width:160px;">${shortKho}</td>
            <td style="font-weight:700;font-family:monospace;white-space:nowrap;">${r.bien_so || '--'}</td>
            <td style="font-size:12px;">${r.loai_xe || '--'}</td>
            <td style="font-size:12.5px;">${r.ncc || '--'}</td>
            <td style="text-align:right;font-weight:600;">${kmDi}</td>
            <td style="text-align:right;font-weight:600;">${kmVe}</td>
            <td style="text-align:right;">${kmMove}</td>
            <td>${statusBadge[r._status] || statusBadge['missing_km']}</td>
            <td>${imgBtn}</td>
            <td style="font-size:11.5px;color:var(--text3);white-space:nowrap;">${r.thoi_gian_update || '--'}</td>
        </tr>`;
    }).join('');
}

// ---- Open image in lightbox ----
function openOdoImage(url, bienSo) {
    const modal = document.getElementById('odo-img-modal');
    const img   = document.getElementById('odo-modal-img');
    const link  = document.getElementById('odo-modal-link');
    if (!modal || !img) return;

    // Convert Google Drive view URL to direct image if possible
    let directUrl = url;
    const m = url.match(/\/d\/([a-zA-Z0-9_-]+)/);
    if (m) directUrl = `https://drive.google.com/thumbnail?id=${m[1]}&sz=w1000`;

    img.src = directUrl;
    img.alt = `Ảnh ODO xe ${bienSo}`;
    if (link) { link.href = url; link.textContent = `Mở ảnh gốc trên Drive (${bienSo}) ↗`; }
    modal.style.display = 'flex';
}
...
function exportOdoExcel() {
    const headers = ['Ngày','ID Kho','Tên Kho','NCC','Biển Số','Loại Xe','KM Đi','KM Về','KM Di Chuyển','Trạng Thái','Link Ảnh','Cập Nhật Lúc'];
    const rows = odoData.map(r => {
        let kmMove = '';
        if (r.km_di && r.km_ve) {
            try { kmMove = (parseFloat(r.km_ve.replace(/[,.]/g,'')) - parseFloat(r.km_di.replace(/[,.]/g,''))).toString(); } catch(e) {}
        }
        const statusMap = { complete: 'Hoàn thành', missing_km: 'Thiếu KM', incomplete: 'Chưa đủ', missing_image: 'Thiếu ảnh', abnormal: 'Bất thường' };
        return [r.ngay, r.id_kho, r.ten_kho, r.ncc, r.bien_so, r.loai_xe, r.km_di, r.km_ve, kmMove, statusMap[r._status] || r._status, r.link_anh, r.thoi_gian_update];
    });
    ...
    a.download = `Bao_Cao_ODO_GXT_${new Date().toLocaleDateString('vi-VN').replace(/\//g,'-')}.csv`;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
}

// Make functions globally accessible
window.loadOdoReport = loadOdoReport;
window.switchOdoTab = switchOdoTab;
window.applyOdoFilters = applyOdoFilters;
window.resetOdoFilters = resetOdoFilters;
window.renderOdoAlerts = renderOdoAlerts;
window.renderOdoHistory = renderOdoHistory;
window.exportOdoExcel = exportOdoExcel;
window.openOdoImage = openOdoImage;
